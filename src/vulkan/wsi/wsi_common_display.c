...
static double
wsi_display_mode_refresh(const struct wsi_display_mode *wsi)
{
...
{
   struct wsi_display_connector *connector =
      vk_zalloc(wsi->alloc, sizeof(struct wsi_display_connector), 8,
                VK_SYSTEM_ALLOCATION_SCOPE_INSTANCE);
                          ...
                          uint32_t connector_id)
{
   ...
   for (int p = 0; p < drm_connector->count_props; p++) {
      drmModePropertyPtr prop = drmModeGetProperty(wsi->fd, drm_connector->props[p]);
      if (!prop)
         continue;
   ...
   for (int m = 0; m < drm_connector->count_modes; m++) {
      VkResult result =
         wsi_display_register_drm_mode(wsi_device, connector,
                            ...
                            wsi_display_mode *display_mode)
{
   ...
   if (!connector->crtc_id) {
      connector->crtc_id =
         wsi_display_select_crtc(connector, mode_res, drm_connector);
      if (!connector->crtc_id) {
         result = VK_ERROR_OUT_OF_DATE_KHR;
         goto bail_connector;
      }
...
wsi_display_fence_alloc(VkDevice                        device,
                        ...
                        const VkAllocationCallbacks     *allocator)
{
   struct wsi_display_fence *fence = vk_zalloc(allocator, sizeof (*fence), 8,
                                               VK_SYSTEM_ALLOCATION_SCOPE_OBJECT);
   ...
   for (;;) {
      ...
      if (ret) {
         ...
         case EINVAL:
            ...
            ret = drmModeSetCrtc(wsi->fd, connector->crtc_id, image->fb_id,
                                 0, 0, &connector->id, 1,
                                   ...
                                   xcb_randr_output_t output)
{
   ...
   if (connector_id_atom == 0) {
      ...
      xcb_intern_atom_cookie_t ia_c =
         xcb_intern_atom(connection, true, 12, "CONNECTOR_ID");
      xcb_intern_atom_reply_t *ia_r =
         xcb_intern_atom_reply(connection, ia_c, NULL);
      if (ia_r) {
   ...
   if (connector_id_atom) {
      ...
      xcb_randr_get_output_property_cookie_t gop_c =
         xcb_randr_get_output_property(connection, output, connector_id_atom,
                                       0, 0, 0xffffffffUL, 0, 0);
                                   ...
                                   uint32_t connector_id)
{
   ...
   for (xcb_screen_iterator_t iter = xcb_setup_roots_iterator(setup);
        output == 0 && iter.rem; xcb_screen_next(&iter)) {
                           ...
                           xcb_randr_output_t output)
{
   ...
   xcb_window_t root = 0;
   for (xcb_screen_iterator_t iter = xcb_setup_roots_iterator(setup);
        root == 0 && iter.rem; xcb_screen_next(&iter)) {
                       ...
                       xcb_randr_output_t output)
{
   ...
   xcb_randr_get_output_info_reply_t *goi_r =
      xcb_randr_get_output_info_reply(connection, goi_c, NULL);

   /* See if we already have a connector for this output */
   struct wsi_display_connector *connector =
      wsi_display_find_output(wsi_device, output);
   ...
   if (goi_r && gsr_r) {
      ...
      connector->connected =
         goi_r->connection != XCB_RANDR_CONNECTION_DISCONNECTED;

      wsi_display_invalidate_connector_modes(wsi_device, connector);

      xcb_randr_mode_t *x_modes = xcb_randr_get_output_info_modes(goi_r);
      for (int m = 0; m < goi_r->num_modes; m++) {
         xcb_randr_mode_info_iterator_t i =
            xcb_randr_get_screen_resources_modes_iterator(gsr_r);
         for (; i.rem; xcb_randr_mode_info_next(&i)) {
            xcb_randr_mode_info_t *mi = i.data;
            if (mi->id == x_modes[m]) {
               VkResult result =
                  wsi_display_register_x_mode(wsi_device, connector, mi,
                                              m < goi_r->num_preferred);
               if (result != VK_SUCCESS)
                  goto fail;
               break;
            }
                         ...
                         VkDisplayKHR display)
{
   ...
   xcb_window_t root = wsi_display_output_to_root(connection,
                                                  connector->output);
   if (!root)
      return VK_ERROR_OUT_OF_DATE_KHR;

   xcb_randr_crtc_t crtc = wsi_display_find_crtc_for_output(connection, root,
                                                            connector->output);
   if (!crtc)
      return VK_ERROR_OUT_OF_DATE_KHR;
   ...
   xcb_randr_create_lease_cookie_t cl_c =
      xcb_randr_create_lease(connection, root, lease, 1, 1, &crtc,
                             &connector->output);
                             ...
                             VkDisplayKHR *display)
{
   ...
   struct wsi_display_connector *connector =
      wsi_display_get_output(wsi_device, connection, output);
   if (connector)
      *display = wsi_display_connector_to_handle(connector);
   else
      *display = NULL;
